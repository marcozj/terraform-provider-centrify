	...
	return &schema.Provider{
		Schema: map[string]*schema.Schema{
			"url": {
				...
				Required:    true,
				DefaultFunc: schema.EnvDefaultFunc("CENTRIFY_URL", ""),
				Description: "Centrify Platform URL",
			},
			"appid": {
				Type:        schema.TypeString,
				Required:    true,
				DefaultFunc: schema.EnvDefaultFunc("CENTRIFY_APPID", ""),
				Description: "Application ID",
			},
			"scope": {
				Type:        schema.TypeString,
				Required:    true,
				DefaultFunc: schema.EnvDefaultFunc("CENTRIFY_SCOPE", ""),
				Description: "OAuth2 scope",
			},
			"username": {
				Type:        schema.TypeString,
				Required:    true,
				DefaultFunc: schema.EnvDefaultFunc("CENTRIFY_USERNAME", ""),
				Description: "Username",
			},
			"password": {
				Type:        schema.TypeString,
				Required:    true,
				DefaultFunc: schema.EnvDefaultFunc("CENTRIFY_PASSWORD", ""),
				Description: "Password",
			},
			"token": {
				Type:        schema.TypeString,
				Required:    true,
				DefaultFunc: schema.EnvDefaultFunc("CENTRIFY_TOKEN", ""),
				Description: "OAuth or DMC token",
			},
			"use_dmc": {
				Type:        schema.TypeBool,
				Required:    true,
				DefaultFunc: schema.EnvDefaultFunc("CENTRIFY_USEDMC", false),
				Description: "Whether to use DMC",
			},
			"logpath": {
				Type:        schema.TypeString,
				Required:    true,
				DefaultFunc: schema.EnvDefaultFunc("CENTRIFY_LOGPATH", ""),
				Description: "Path of log file",
			},
			"skip_cert_verify": {
				Type:        schema.TypeBool,
				Required:    true,
				DefaultFunc: schema.EnvDefaultFunc("CENTRIFY_SKIPCERTVERIFY", false),
			...
			"log_level": {
				...
				DefaultFunc: schema.EnvDefaultFunc("CENTRIFY_LOGLEVEL", "Error"),
			},
		},
