data "centrifyvault_passwordprofile" "checkpoint_pw_pf" {
    name = "Check Point Gaia Profile"
}

# TODO: administrative_account_id setting
resource "centrifyvault_vaultsystem" "checkpoint2" {
    # System -> Settings menu related settings
    name = "Demo Checkpoint 2"
    fqdn = "192.168.2.4"
    computer_class = "CheckPointGaia"
    session_type = "Ssh"
    description = "Demo Checkpoint 2"
    port = 22
    system_timezone = "UTC"
    ...
    password_profile_id = data.centrifyvault_passwordprofile.checkpoint_pw_pf.id
