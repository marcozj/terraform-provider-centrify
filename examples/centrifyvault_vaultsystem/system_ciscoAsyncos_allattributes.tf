data "centrifyvault_passwordprofile" "asyncos_pw_pf" {
    name = "Cisco AsyncOS Profile"
}

# TODO: administrative_account_id setting
resource "centrifyvault_vaultsystem" "ciscoasyncos2" {
    # System -> Settings menu related settings
    name = "Demo CiscoAsyncOS 2"
    fqdn = "192.168.2.4"
    computer_class = "CiscoAsyncOS"
    session_type = "Ssh"
    description = "Demo CiscoAsyncOS 2"
    port = 22
    system_timezone = "UTC"
    ...
    password_profile_id = data.centrifyvault_passwordprofile.asyncos_pw_pf.id
