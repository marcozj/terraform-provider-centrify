data "centrifyvault_passwordprofile" "cisconxos_pw_pf" {
    name = "Cisco NX-OS Profile"
}

resource "centrifyvault_vaultsystem" "cisconxos2" {
    # System -> Settings menu related settings
    name = "Demo CiscoNXOS 2"
    fqdn = "192.168.2.4"
    computer_class = "CiscoNXOS"
    session_type = "Ssh"
    description = "Demo CiscoNXOS 2"
    port = 22
    system_timezone = "UTC"
    ...
    password_profile_id = data.centrifyvault_passwordprofile.cisconxos_pw_pf.id
