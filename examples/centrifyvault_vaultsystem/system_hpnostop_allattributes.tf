data "centrifyvault_passwordprofile" "hp_pw_pf" {
    name = "HP NonStop Profile"
}
# TODO: administrative_account_id and local_account_automatic_maintenance setting
resource "centrifyvault_vaultsystem" "hpnonstop2" {
    # System -> Settings menu related settings
    name = "Demo HP Nonstop 2"
    fqdn = "192.168.2.2"
    computer_class = "HpNonStopOS"
    session_type = "Ssh"
    description = "Demo HP Nonstop system 2"
    port = 22
    ...
    password_profile_id = data.centrifyvault_passwordprofile.hp_pw_pf.id
