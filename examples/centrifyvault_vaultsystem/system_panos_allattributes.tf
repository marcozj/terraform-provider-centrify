data "centrifyvault_passwordprofile" "panos_pw_pf" {
    name = "Palo Alto Networks PAN-OS Profile"
}

# TODO: administrative_account_id setting
resource "centrifyvault_vaultsystem" "panos2" {
    # System -> Settings menu related settings
    name = "Demo Palo Alto PAN-OS 2"
    fqdn = "192.168.2.4"
    computer_class = "PaloAltoNetworksPANOS"
    session_type = "Ssh"
    description = "Demo Palo Alto PAN-OS 2"
    port = 22
    system_timezone = "UTC"
    management_port = 443
    ...
    password_profile_id = data.centrifyvault_passwordprofile.panos_pw_pf.id
