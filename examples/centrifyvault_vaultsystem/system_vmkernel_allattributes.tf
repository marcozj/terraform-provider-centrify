data "centrifyvault_passwordprofile" "vmkernel_pw_pf" {
    name = "VMware VMkernel Profile"
}

# TODO: administrative_account_id setting
resource "centrifyvault_vaultsystem" "vmkernel2" {
    # System -> Settings menu related settings
    name = "Demo VMware VMkernel 2"
    fqdn = "192.168.2.4"
    computer_class = "VMwareVMkernel"
    session_type = "Ssh"
    description = "Demo VMware VMkernel 2"
    port = 22
    system_timezone = "UTC"
    management_port = 443
    ...
    password_profile_id = data.centrifyvault_passwordprofile.vmkernel_pw_pf.id
